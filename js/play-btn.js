	...
	.on('click', function(){
		...
		if (firstPlay){
			...
			function addCapContent(plr, num){
				var color = num == '1' ? 'white' : 'white';
				var colorGray = num == '1' ? 'white' : 'black';
			...
			// winMsg.hideMsg()
			if (WIN.isWin){
				winMsg.showMsg('Le temps pour une autre partie...')
			} else {
				d3.select('.msg-text').text('Jouez encore')
			}
			WIN = {}
