var winMsg = function (){
	...
	self.showWhiteMsg = function(text){
		self.showMsg(text, '#f9f9f9', '#f9f9f9')
	}

	self.showBlackMsg = function(text){
		self.showMsg(text,'#17202A','#17202A')
	}
	...
	self.showMsg = function(text, backColor, frontColor, textColor){
		...
		backColor = backColor || '#030306';
		frontColor = frontColor || '#030306';
...
setTimeout(function(){
	winMsg.showMsg('Bienvenue sur Pente')

	
}, 1000)

// winMsg.showRedMsg('The Computer Wonnnnn :(')
// // winMsg.showRedMsg('The Computer Wonnnnn :(')
